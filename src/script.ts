...
function updateWallpaperUIState(enabled: boolean, animate = true): void {
  ...
  const container =
    ...
    (wallpaperSourceSelect ? wallpaperSourceSelect.closest(".wallpaper-source-options") : null);
  if (container) {
    ...
    const label = (document.querySelector(`label[for="${wallpaperSourceSelect.id}"]`) ||
      (wallpaperSourceSelect.previousElementSibling && wallpaperSourceSelect.previousElementSibling.tagName === "LABEL"
  ...
  const overlaySetting =
    wallpaperOverlaySetting || (overlaySliderContainer?.closest(".collapsible-setting") as HTMLElement | null);
  if (overlaySetting) {
...
function bindExternalShortcutDrop(): void {
  ...
  shortcutsGrid.addEventListener("dragover", (event) => {
    ...
    const hasUrl = Array.from(event.dataTransfer.types).some(
      (type) => type === "text/uri-list" || type === "text/plain" || type === "text/html",
    );
    if (!hasUrl) return;
    event.preventDefault();
...
function bindLauncherFolderButton(): void {
  ...
  btnLauncherToFolder.addEventListener("click", (event) => {
    ...
    warningModal.show({
      title: getLocalizedWarningText("warningAddEcosystemAppsTitle", `Add ${ecosystemName} Apps?`, {
...
async function getUpdateNoticeState(): Promise<{ pending: boolean; version: string }> {
  ...
  const storedVersion =
    typeof items[UPDATE_NOTICE_VERSION_KEY] === "string" ? String(items[UPDATE_NOTICE_VERSION_KEY]) : "";
