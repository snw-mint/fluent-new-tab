...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const starSvg = (type) => {
    const common =
      'width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" class="star-icon star-' +
      type +
      '"';
  ...
  const animateValue = (element, start, end, duration, isFloat = false) => {
    ...
    const step = (timestamp) => {
      ...
      if (isFloat) {
        ...
        element.textContent = formatCount(Math.floor(currentVal));
      }

      if (progress < 1) {
        window.requestAnimationFrame(step);
      } else {
        element.textContent = isFloat ? end.toFixed(1) : formatCount(end);
      }
    };
    window.requestAnimationFrame(step);
  };
      ...
      const ratingsObserver = new IntersectionObserver(
        (entries) => {
          if (entries[0].isIntersecting) {
            if (usersNode) animateValue(usersNode, 0, userCount, 2000);
            if (ratingNode) animateValue(ratingNode, 0, ratingValue, 2000, true);
            if (reviewsNode) animateValue(reviewsNode, 0, reviewCount, 2000);

            if (starsNode) starsNode.classList.add("animate-stars");
